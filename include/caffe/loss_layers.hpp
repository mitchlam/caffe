 ...
 */
template <typename Dtype>
class WrapperContrastiveLossLayer : public LossLayer<Dtype> {
 public:
  explicit WrapperContrastiveLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline ~WrapperContrastiveLossLayer(){
  ...
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline const char* type() const { return "WrapperContrastiveLoss"; }

 protected:
  /// @copydoc WrapperContrastiveLossLayer
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  //virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  //    const vector<Blob<Dtype>*>& top);
...
//Wrapper Contrastive Loss
Blob<Dtype> bottom_; //Original data
Blob<Dtype> bottom_p_; //offset data 
Blob<Dtype> sims_;  //0,1 for similarity
vector<Blob<Dtype>*> bottom_vec_;  //Vector for bottom blobs

Blob<Dtype> top_;
vector<Blob<Dtype>*> top_vec_; //vector for top

vector<ContrastiveLossLayer<Dtype>*> layers_; //vector of Loss layers


/* BatchContrastiveLoss
  Blob<Dtype> diff_;  // cached for backward pass
  Blob<Dtype> dist_;  // cached distance
  Blob<Dtype> m_dist_; //cached margined distance or 0.0 or similar pairs
  Blob<Dtype> diff_sq_;  // tmp storage for gpu forward pass
  Blob<Dtype> temp_; // tmp storage for cpu calculations
  Blob<Dtype> sims_; //cached sim score
  Blob<Dtype> deriv_; // Derivative accumulator
  Blob<Dtype> summer_vec_; // vector to sum along N
  
 
  //Blob<Dtype> input_T_; //tmp storage for transposed input
