...
class WrapperContrastiveLossLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  WrapperContrastiveLossLayerTest()
    ...
    for (int i = 0; i < blob_bottom_y_->count(); ++i) {
      blob_bottom_y_->mutable_cpu_data()[i] = caffe_rng_rand() % 4;  // 4 classes
    }
    blob_bottom_vec_.push_back(blob_bottom_y_);
    blob_top_vec_.push_back(blob_top_loss_);
  }
  virtual ~WrapperContrastiveLossLayerTest() {
...
TYPED_TEST_CASE(WrapperContrastiveLossLayerTest, TestDtypesAndDevices);
/*
TYPED_TEST(WrapperContrastiveLossLayerTest, TestForward) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  WrapperContrastiveLossLayer<Dtype> layer(layer_param);
...
*/
TYPED_TEST(WrapperContrastiveLossLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  WrapperContrastiveLossLayer<Dtype> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
...
TYPED_TEST(WrapperContrastiveLossLayerTest, TestGradientLegacy) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.mutable_contrastive_loss_param()->set_legacy_version(true);
  WrapperContrastiveLossLayer<Dtype> layer(layer_param);
